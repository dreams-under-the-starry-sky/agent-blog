...
async function load() {
  all.value = await adminApi.recordCategories()
}
...
async function save() {
  ...
  try {
    await adminApi.saveRecordCategory({ ...form, name })
...
async function remove(id: number) {
  ...
  await adminApi.deleteRecordCategory(id)
...
<template>
  ...
  >
    ...
    <template #extra>
      <el-dialog v-model="visible" title="记录分类" width="400px" align-center>
